    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                ...
                DynamicTanh(normalized_shape=dim, channels_last=True, alpha_init_value=0.5),
                Cross_attention(dim, num_heads=heads,bias=False),
                PreNorm(dim, FeedForward(dim=dim))
            ]))
        
        self.s_conv=  nn.Conv2d(3, dim, kernel_size=3, stride=1, padding=1)
        self.s_norm = DynamicTanh(normalized_shape=dim, channels_last=True, alpha_init_value=0.5)

    def forward(self, x, s):
        """
        x: [b,c,h,w]
        """
        s = self.s_norm(self.s_conv(s).permute(0, 2, 3, 1))
        
        x = x.permute(0, 2, 3, 1)
        for (attn, cross_norm, cross_attn, ff) in self.blocks:
            x = attn(x) + x
            x = cross_attn(cross_norm(x), s.permute(0, 3, 1, 2)) + x
            x = ff(x) + x
