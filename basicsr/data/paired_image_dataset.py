...
class Dataset_PairedImage(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.gt_folder_s, self.lq_folder_s = opt['dataroot_gt_s'], opt['dataroot_lq_s']
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            self.paths_s = paired_paths_from_folder(
                [self.lq_folder_s, self.gt_folder_s], ['lq_s', 'gt_s'],
                self.filename_tmpl)
    ...
    def __getitem__(self, index):
        ...
        gt_path_s = self.paths_s[index]['gt_s_path']
        img_bytes = self.file_client.get(gt_path_s, 'gt_s')
        try:
            img_gt_s = imfrombytes(img_bytes, float32=True)
        except:
            raise Exception("gt_s path {} not working".format(gt_path_s))

        lq_path_s = self.paths_s[index]['lq_s_path']
        img_bytes = self.file_client.get(lq_path_s, 'lq_s')
        try:
            img_lq_s = imfrombytes(img_bytes, float32=True)
        except:
            raise Exception("lq_s path {} not working".format(lq_path_s))
        ...
        if self.opt['phase'] == 'train':
            ...
            # padding
            img_gt, img_lq, img_gt_s, img_lq_s = padding(img_gt, img_lq, img_gt_s, img_lq_s, gt_size)

            # random crop
            img_gt, img_lq, img_gt_s, img_lq_s = paired_random_crop(img_gt, img_lq, img_gt_s, img_lq_s, gt_size, scale,
                                                gt_path)

            # flip, rotation augmentations
            if self.geometric_augs:
                img_gt, img_lq, img_gt_s, img_lq_s = random_augmentation(img_gt, img_lq, img_gt_s, img_lq_s)
            
        # BGR to RGB, HWC to CHW, numpy to tensor
        img_gt, img_lq, img_gt_s, img_lq_s = img2tensor([img_gt, img_lq, img_gt_s, img_lq_s],
                                    bgr2rgb=True,
                                    float32=True)
        # normalize
