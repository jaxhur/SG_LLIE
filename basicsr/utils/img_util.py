...
def padding(img_lq, img_gt, img_lq_s, img_gt_s, gt_size):
    h, w, _ = img_lq.shape
    ...
    if h_pad == 0 and w_pad == 0:
        return img_lq, img_gt, img_lq_s, img_gt_s

    img_lq = cv2.copyMakeBorder(img_lq, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_gt = cv2.copyMakeBorder(img_gt, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)

    img_lq_s = cv2.copyMakeBorder(img_lq_s, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    img_gt_s = cv2.copyMakeBorder(img_gt_s, 0, h_pad, 0, w_pad, cv2.BORDER_REFLECT)
    ...
    if img_lq_s.ndim == 2:
        img_lq_s = np.expand_dims(img_lq_s, axis=2)
    if img_gt_s.ndim == 2:
        img_gt_s = np.expand_dims(img_gt_s, axis=2)
    return img_lq, img_gt, img_lq_s, img_gt_s
